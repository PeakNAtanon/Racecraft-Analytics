...
export interface WeatherSnapshot { source: "FastF1" | "OpenF1" | "fallback"; sessionName: string; sampledAt?: string; airTemperature?: number; trackTemperature?: number; humidity?: number; windSpeed?: number; windDirection?: number; rainfall?: boolean }
...
export type FastF1DriverDataStatus = "complete" | "no_laps" | "no_valid_laps" | "no_telemetry";
export interface FastF1DriverAvailability { status: FastF1DriverDataStatus; validLaps: number; telemetrySamples: number; reason: string }
export interface SessionAnalyticsSnapshot { sessionKey?: number; sessionName: string; source: "FastF1" | "OpenF1" | "fallback"; metrics: Metric[]; driverMetrics?: Record<string, FastF1DriverMetrics>; driverAvailability?: Record<string, FastF1DriverAvailability>; racecraftByDriver?: Record<string, DriverRacecraftSnapshot>; telemetryByDriver?: Record<string, DriverTelemetrySnapshot>; pace: PaceChartData; stints: StintSnapshot[]; results: SessionResult[]; resultsSource: "OpenF1" | "Jolpica" | "fallback"; weather?: WeatherSnapshot }
