...
export default async function Compare({ searchParams }: { searchParams?: Promise<{ drivers?: string | string[] }> }) {
  ...
  const latestSession = latestFastF1Session(comparison.sessions);
  ...
  return <><PageHead eyebrow="COMPARISON LAB · FASTF1" title="Compare">{message(locale, "compareDescription")}</PageHead><CompareLab drivers={comparison.drivers} pace={analytics.pace} stints={analytics.stints} comparison={comparison} locale={locale} initialDriverCodes={requestedDrivers} /></>;
}
