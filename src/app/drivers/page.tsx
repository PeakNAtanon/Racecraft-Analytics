      ...
      <div className="driver-analysis-grid-hub">
        {visibleDrivers.map((driver) => {
          ...
          return <article className="driver-analysis-card-link driver-analysis-card-hub" key={driver.code} style={{ "--driver-color": color } as React.CSSProperties}>
              ...
              <div className="driver-hub-identity"><div className="team-mark" aria-hidden="true">{getTeamMark(driver.team)}</div><div><h3><Link href={`/drivers/${driver.code.toLowerCase()}?season=${season}`}>{driver.name}</Link></h3><p><b>{driver.code}</b> · {driver.team}</p></div></div>
              ...
              <div className="driver-card-actions"><Link href={`/drivers/${driver.code.toLowerCase()}?season=${season}`} className="link-arrow">VIEW ANALYSIS →</Link><Link href={`/compare?season=${season}&drivers=${encodeURIComponent(driver.code)}`} className="driver-compare-link">COMPARE ↗</Link></div>
          </article>;
        })}
