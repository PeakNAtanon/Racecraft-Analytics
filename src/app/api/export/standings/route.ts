...
export async function GET() {
  ...
  if (snapshot.source === "fallback") {
    return Response.json({ error: "Published standings are currently unavailable. Please try again later." }, { status: 503, headers: { "cache-control": "no-store", "retry-after": "600" } });
  }
