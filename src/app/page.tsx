...
export default async function Home() {
  ...
  const [news, standingsSnapshot, analytics, circuitStats] = await Promise.all([getNews(), getSeasonStandings(), getSessionAnalytics({ fastF1Only: false }), getCircuitStats(round)]);
  ...
  return <>
    ...
    <AdSlot />
    <section className="section analytics-section"><div className="section-heading"><div><div className="eyebrow">{analytics.source === "FastF1" ? "FASTF1 ANALYSIS" : "SESSION PACE · OPENF1"}</div><h2>{analytics.source === "FastF1" ? message(locale, "homeAnalyticsTitle") : "Session pace snapshot"}</h2></div><p>{analytics.source === "FastF1" ? "FASTF1 VALIDATED ANALYSIS" : analytics.source === "OpenF1" ? "OPENF1 SESSION CONTEXT · PROVISIONAL" : "FASTF1 ANALYSIS · PENDING"}<br /><span>{analytics.pace.sessionLabel}</span></p></div><MetricGrid items={analytics.metrics} /></section>
