...
function TyrePerformanceChart({ stints, activeDrivers }: { stints: StintSnapshot[]; activeDrivers: string[] }) {
  ...
  const option = { backgroundColor: "transparent", animation: false, aria: { enabled: true }, grid: { left: 54, right: 24, top: 20, bottom: 64, containLabel: true }, tooltip: { trigger: "axis", confine: true, backgroundColor: `${theme.surface}f7`, borderColor: `${theme.cyan}66`, textStyle: { color: theme.text, fontFamily: "JetBrains Mono, monospace", fontSize: 11 }, valueFormatter: (value: unknown) => chartTooltipValue(value, " s") }, xAxis: { type: "category", data: points.map(point => `${point.code} S${point.stint}`), axisLabel: { color: theme.muted, fontSize: 9, fontFamily: "JetBrains Mono, monospace", rotate: 35, hideOverlap: true }, axisLine: { lineStyle: { color: theme.line } }, axisTick: { show: false } }, yAxis: { type: "value", name: "STINT MEDIAN · SEC", nameTextStyle: { color: theme.muted, fontSize: 9, fontFamily: "JetBrains Mono, monospace" }, axisLabel: { color: theme.muted, fontSize: 10, fontFamily: "JetBrains Mono, monospace" }, axisLine: { show: false }, splitLine: { lineStyle: { color: "#ffffff14", type: "dashed" } } }, series: [{ name: "Stint pace", type: "line", data: points.map(point => ({ value: point.medianLap, itemStyle: { color: compoundColor(point.compound) } })), showSymbol: true, symbolSize: 8, connectNulls: false, lineStyle: { color: theme.cyan, width: 2 }, itemStyle: { borderColor: theme.background, borderWidth: 2 } }] };
