...
function FormChart({ sessions }: { sessions: DriverAnalysisSession[] }) {
  ...
  const option = {
    ...
    tooltip: { trigger: "axis", confine: true, backgroundColor: `${theme.surface}f7`, borderColor: `${theme.cyan}66`, textStyle: { color: theme.text, fontFamily: "JetBrains Mono, monospace", fontSize: 11 }, valueFormatter: (item: unknown) => typeof item === "number" ? `P${item}` : "—" },
    grid: { left: 48, right: 22, top: 22, bottom: 46, containLabel: true },
    xAxis: { type: "category", data: rows.map((session) => `${session.circuit}\n${session.sessionCode}`), axisLabel: { color: theme.muted, fontSize: 9, fontFamily: "JetBrains Mono, monospace", interval: 0, rotate: rows.length > 7 ? 28 : 0 }, axisLine: { lineStyle: { color: theme.line } }, axisTick: { show: false } },
    yAxis: { type: "value", inverse: true, min: 1, name: "FINISH", nameTextStyle: { color: theme.muted, fontSize: 9, fontFamily: "JetBrains Mono, monospace" }, axisLabel: { color: theme.muted, fontSize: 10, fontFamily: "JetBrains Mono, monospace", formatter: (item: unknown) => `P${item}` }, axisLine: { show: false }, splitLine: { lineStyle: { color: "#ffffff14", type: "dashed" } } },
...
function RacecraftChart({ sessions }: { sessions: DriverAnalysisSession[] }) {
  ...
  const option = {
    backgroundColor: "transparent", animation: false, aria: { enabled: true },
    tooltip: { trigger: "axis", axisPointer: { type: "shadow" }, confine: true, backgroundColor: `${theme.surface}f7`, borderColor: `${theme.red}66`, textStyle: { color: theme.text, fontFamily: "JetBrains Mono, monospace", fontSize: 11 }, valueFormatter: (item: unknown) => typeof item === "number" ? `${item > 0 ? "+" : ""}${item} positions` : "—" },
    grid: { left: 45, right: 22, top: 20, bottom: 42, containLabel: true },
    xAxis: { type: "category", data: rows.map((session) => `${session.circuit}\n${session.sessionCode}`), axisLabel: { color: theme.muted, fontSize: 9, interval: 0, rotate: rows.length > 7 ? 28 : 0 }, axisLine: { lineStyle: { color: theme.line } }, axisTick: { show: false } },
    yAxis: { type: "value", name: "GRID → FINISH", nameTextStyle: { color: theme.muted, fontSize: 9 }, axisLabel: { color: theme.muted, fontSize: 10, formatter: (item: unknown) => `${Number(item) > 0 ? "+" : ""}${item}` }, axisLine: { show: false }, splitLine: { lineStyle: { color: "#ffffff14", type: "dashed" } } },
...
export function DriverAnalysisView({ snapshot, locale, timezone, filters }: { snapshot: DriverAnalysisSnapshot; locale: Locale; timezone: string; filters?: { season?: string; round?: string; circuit?: string; sessionCode?: string; sessionKey?: string } }) {
  ...
  return <div className="driver-analysis-page" style={{ "--driver-color": color } as React.CSSProperties}>
    ...
    <nav className="driver-section-nav" aria-label="Driver analysis sections"><span>JUMP TO</span><a href="#driver-overview">Overview</a><a href="#driver-form">Form</a><a href="#driver-pace">Pace</a><a href="#driver-strategy">Strategy</a><a href="#driver-telemetry">Telemetry</a><a href="#driver-sessions">Sessions</a></nav>
    ...
    <section className="section driver-analysis-grid"><article id="driver-form" className="panel driver-analysis-card driver-form-card"><div className="section-heading"><div><div className="eyebrow">FORM TREND</div><h2>Race finishes</h2></div><p>JOLPICA · RESULTS</p></div><FormChart sessions={snapshot.sessions} /></article><article className="panel driver-analysis-card"><div className="section-heading"><div><div className="eyebrow">RACECRAFT</div><h2>Grid to finish</h2></div><p>{racecraft?.source === "FastF1" ? "FASTF1 · VALIDATED" : "FASTF1 · PENDING"}</p></div><RacecraftChart sessions={snapshot.sessions} /><div className="driver-racecraft-stats"><span><small>FASTF1 POSITION DELTA</small><b>{racecraft?.positionsGained === undefined ? "—" : `${racecraft.positionsGained >= 0 ? "+" : ""}${racecraft.positionsGained}`}</b></span><span><small>OVERTAKES MADE</small><b>{racecraft?.overtakesMade ?? "—"}</b></span><span><small>OVERTAKES LOST</small><b>{racecraft?.overtakesLost ?? "—"}</b></span><span><small>RACE CONTROL</small><b>{racecraft?.raceControlEvents ?? "—"}</b></span></div></article></section>

    <section id="driver-pace" className="section panel driver-pace-panel"><div className="section-heading"><div><div className="eyebrow">PACE ANALYSIS</div><h2>{session ? `${session.circuit} · ${session.sessionCode}` : "Selected session pace"}</h2></div><p>{context === "teammate" ? "TEAMMATE CONTEXT" : "SESSION FIELD"}<br />{selectedAnalytics?.source === "FastF1" ? "FASTF1 · VALIDATED ANALYSIS" : selectedAnalytics?.source === "OpenF1" ? "OPENF1 · PROVISIONAL CONTEXT" : "PROVIDER DATA UNAVAILABLE"}</p></div>{fastF1Metrics ? <div className="driver-fastf1-metrics" aria-label="FastF1 validated driver metrics"><article><span>CLEAN-LAP MEDIAN</span><strong>{fastF1Seconds(fastF1Metrics.cleanLapMedian)}</strong></article><article><span>BEST LAP</span><strong>{fastF1Seconds(fastF1Metrics.bestLap)}</strong></article><article><span>THEORETICAL BEST</span><strong>{fastF1Seconds(fastF1Metrics.theoreticalBest)}</strong></article><article><span>CONSISTENCY</span><strong>{fastF1Seconds(fastF1Metrics.consistency)}</strong></article><article><span>DEGRADATION</span><strong>{fastF1Seconds(fastF1Metrics.degradationSlope)}</strong></article><article><span>VALID LAPS</span><strong>{fastF1Metrics.validLaps ?? "—"}</strong></article></div> : null}{paceData ? <PaceChart data={paceData} /> : <div className="empty">Select a completed session to inspect FastF1 pace. Metrics appear after a validated artifact is published.</div>}</section>
